...
const button = document.querySelector('.google-page__btn');
...
function superSum (a,b){   //№6
	let c = a + b;
	return (c)
}

function minNumber(){   //№7
	myArray=[2,1,4,3,6,5,55,100,22]
	min = myArray[0];
...
function findMax(someParams){               //№9
	let max = someParams[0]
...
findMax([33,44,345,65,21,4,5,6,33,1])

button.addEventListener("click", function(e) {           //№3,4,10
	const conclusion =  inputIn.value;
	if (conclusion === 'google'){
	setTimeout("alert('Yandex круче. Но это не точно')",3000);
		inputIn.value = '';
	}else{
			
	alert(conclusion + ', first name: ' + mass[0].name + ', superSumm: ' + superSum (2,2) + ', Max: '  + maxNumber() + ', Min: ' + minNumber());
	inputIn.value = '';
	}	
});
